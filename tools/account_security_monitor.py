...
"""
账户安全监控工具
用于监控账户安全相关事件，包括异常登录、权限变更、账户锁定等
...
| where event.action in ("Add", "Delete") AND fortinet.firewall.cfgpath="user.local"
...
| where out5="keep"
| fields fortinet.firewall.subtype, source.user.name, source.ip, fortinet.firewall.srccountry, @timestamp, @gid, event.action, fortinet.firewall.cfgpath, observer.name, fortinet.firewall.cfgobj, message, fortinet.firewall.ui, uiscrip, rule.description, observer.serial_number, rule.id"""
...
def _calculate_ip_priority(raw_result: dict) -> list:
    """根据账户安全事件的操作者 IP（uiscrip）计算优先级"""
    try:
        data = raw_result.get("data", [])
        logger.info(f"[AccountTrace] 开始计算 IP 优先级，原始数据条数: {len(data)}")
        if not data:
            logger.warning("[AccountTrace] 未检测到数据，跳过 IP 优先级计算")
            return []
        ...
        logger.info(f"[AccountTrace] 统计到的 IP 分布: {ip_counts}")
        if not ip_counts:
            logger.warning("[AccountTrace] 未找到有效的 IP 字段")
            return []
        ...
        for ip, count in sorted_ips:
            ...
            trace_ips.append(ip)
            logger.info(f"[AccountTrace] 已收录 IP {ip} (累计占比: {cumulative:.2%})")
            if cumulative >= 0.8 or len(trace_ips) >= 5:
                logger.info(f"[AccountTrace] 达到阈值，停止选取")
                break
        
        logger.info(f"[AccountTrace] 最终选取的溯源 IP 列表: {trace_ips}")
        return trace_ips
        
    except Exception as e:
        logger.error(f"[AccountTrace] 计算 IP 优先级失败: {e}", exc_info=True)
        return []
...
def _auto_trace_account_security_ips(raw_result: dict, start_time: str, end_time: str, gid: str) -> dict:
    """自动对账户安全事件的操作者 IP 执行溯源查询"""
    try:
        logger.info("[AccountTrace] 开始执行自动溯源查询...")
        trace_ips = _calculate_ip_priority(raw_result)
        
        if not trace_ips:
            logger.info("[AccountTrace] 未检测到操作者 IP，跳过溯源")
            return {"status": "success", "message": "未检测到操作者 IP，无需溯源", "ip_details": []}
        
        logger.info(f"[AccountTrace] 开始对 {len(trace_ips)} 个 IP 执行溯源查询")
        ip_details = []
        ...
        for ip in trace_ips:
            try:
                ...
                logger.info(f"[AccountTrace] IP {ip} 溯源查询完成，状态: {trace_result.get('trace_info', {}).get('status')}")
            except Exception as e:
                logger.error(f"[AccountTrace] IP {ip} 溯源查询失败: {e}", exc_info=True)
        ...
        logger.info(f"[AccountTrace] 自动溯源完成，共处理 {len(ip_details)} 个 IP")
        return result
        
    except Exception as e:
        logger.error(f"[AccountTrace] 自动溯源执行失败: {e}", exc_info=True)
        return {"status": "error", "message": f"自动溯源执行失败：{str(e)}", "ip_details": []}



def account_security_monitor_request(
    user_problem: str = "",
    start_time: str = None,
    end_time: str = None,
    filter_ip: str = None,
    ...
    Args:
        user_problem: 用户问题描述，支持：今天、近 7 天、IP 过滤、用户名过滤等
        start_time: 开始时间（可选）
        end_time: 结束时间（可选）
        filter_ip: 过滤的 IP 地址
        filter_user: 过滤的用户名
    ...
    executor = ToolExecutor(
        ...
        ip_field="uiscrip",  # 账户安全使用 uiscrip 字段（从 UI 字段解析出的操作者 IP）
    )
    ...
    try:
        ...
        logger.error(f"[AccountSecurity] 解析结果失败: {e}")
        return json.dumps({
            "status": "error",
    ...
    # 自动触发溯源查询
    try:
        trace_info = _auto_trace_account_security_ips(raw_result, start_time, end_time, gid)
    except Exception as e:
        logger.error(f"[AccountSecurity] 自动溯源执行异常: {e}", exc_info=True)
...
class AccountSecurityMonitorInput(BaseModel):
    """账户安全监控工具输入参数模型"""
    user_problem: str = Field(default="", description="用户的问题描述")
